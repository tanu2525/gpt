...
exports.getZohoOAuthUrl = async function(req, res) {
    try {
        ...
        const url = zohoOAuthService.createAuthorizationUrl(organizationId, apiDomain);
...
exports.zohoOAuthCallback = async function(req, res) {
    try {
        ...
        const token = await zohoOAuthService.exchangeCode(code, stateData.redirectUri);

        if (!token.refresh_token) {
            return res.status(400).send("Zoho did not return a refresh token. Make sure access_type=offline is used.");
        }
        ...
        return res.status(error.statusCode || error.response?.status || 500).send(
...
exports.saveWorkflow = async function(req, res) {
    try {
        ...
        const { autoConfigureZoho = false, ...workflowData } = req.body;
        ...
        const workflow = await WorkflowConfig.findOneAndUpdate(
            { organizationId, workflowName: req.body.workflowName },
            {
                ...workflowData,
                organizationId,
                triggerType: req.body.triggerType || req.body.trigger,
                enabled: true
            },
        ...
        return res.status(201).json({ success: true, workflowId: workflow._id, workflow, zoho: zohoSetup });
    } catch (error) {
...
exports.zohoNotification = async function(req, res) {
    ...
    try {
        ...
        if (!channelId) {
            console.warn("Zoho notification ignored: missing channel_id");
            return;
        }

        const workflow = await WorkflowConfig.findOne({ zohoChannelId: channelId, enabled: true })
            .select("+zohoNotificationToken");

        if (!workflow) {
            console.warn("No workflow found for Zoho channel:", channelId);
            return;
        }

        if (workflow.zohoNotificationToken && payload.token !== workflow.zohoNotificationToken) {
            console.warn("Invalid Zoho notification token for channel:", channelId);
            return;
        }

        const incomingModule = String(payload.module || "");
        if (incomingModule && incomingModule !== workflow.module) {
            console.warn("Zoho notification module mismatch:", incomingModule, workflow.module);
            return;
        }

        const incomingTrigger = zohoNotificationService.operationToTrigger(payload.operation);
        const configuredTrigger = String(workflow.triggerType || workflow.trigger).toLowerCase();

        if (configuredTrigger !== "all" && configuredTrigger !== "create_or_edit" && incomingTrigger !== configuredTrigger) {
            console.warn("Zoho notification trigger mismatch:", incomingTrigger, configuredTrigger);
            return;
        }

        const ids = Array.isArray(payload.ids) ? payload.ids : [];
        if (!ids.length) {
            console.warn("Zoho notification contained no record IDs.");
            return;
        }
        ...
        for (const recordId of ids) {
            try {
                ...
                console.error(`Workflow ${workflow._id} failed for record ${recordId}:`, error.response?.data || error.message);
            }
        }
    } catch (error) {
...
// Temporary OAuth test endpoint; remove after OAuth setup is verified.
exports.testOAuth = async (req, res) => {
    const organizationId = "4599126000000295996";
    const url = zohoOAuthService.createAuthorizationUrl(organizationId);
    res.send(`
        <h2>Zoho Sandbox OAuth</h2>
        <a href="${url}" target="_blank">Connect Sandbox CRM</a>
    `);
};
